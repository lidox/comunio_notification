...
public class CommunioClientBot implements Bot {
   ...
   /**
    * Log in into comunio with the login data from config file or object
    * 
    * @param mobile
    *           decide whether the mobile or the desktop version will be used
    */
   public HtmlPage login(boolean mobile) throws FailingHttpStatusCodeException,
   ...
   public ArrayList<ComunioPlayer> getTeamFormationAsList() {
      ...
      try {
         ...
         int index =  ret.getUrl().toString().indexOf("sta/");
         String sessionID = ret.getUrl().toString().substring(index+4);
         String beginning = "http://m.comunio.de/li2/";
         URL url = new URL(beginning+sessionID);
         ret = getWebClient().getPage(url);

         String res = ret.asXml();

         int start = res
               .indexOf("<div style=\"width:96px;background-image");//<div style=\"width:px;height:px;background-image:url
         ...
         for (int i = 0; i < playerCount; i++) {
            ...
            StringBuilder aPlayer = new StringBuilder(information.substring(
                  startPattern, endPattern));
            String playerName = getValueByEdges(aPlayer,
                  "height:20px;line-height:10px;\">", "<span style=\"font-size:xx-small").trim();//</div>
            String weblink = getValueByEdges(aPlayer, "<a href=\"", "\" style")
                  .trim();
            String position = "";
